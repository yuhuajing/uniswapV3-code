...
library Tick {
    ...
    function update(
        ...
        Tick.Info storage tickInfo = self[tick];  //记录全局流动性和当前交易中的流动性数据
        ...
        if (liquidityBefore == 0) {
            ...
            // the tick
            if (tick <= currentTick) { // 小于当前Tick，OutSide值为 global_old ; 大于 当前Tick, Outside的值为 0； crossTick的时候，需要进行反转， 因为outSide始终记录dangqianTick的反向范围的Fee
